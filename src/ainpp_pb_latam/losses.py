...
class CenterCropLoss(nn.Module):
    """
    Wraps any (pred, target)-style loss module and center-crops both tensors
    to `crop_size` (a single int, applied to both H and W) before delegating
    to `base_loss`.

    Motivation: LUPIN's `_extrapolate` operator (semi-Lagrangian warp via
    repeated `grid_sample`) uses `padding_mode="zeros"`/`"border"`, so pixels
    near the domain border are progressively less reliable the more warp
    steps they go through (the reference implementation crops a 48px margin
    at 336x336 for exactly this reason). This loss lets any stage discard
    that margin during training without touching the model's own output
    shape or the benchmark's evaluation pipeline (cropping happens purely
    inside the loss). Works with the 5D (B, T, C, H, W) tensors used
    throughout this benchmark, since cropping only ever affects the last two
    (H, W) dimensions.

    Set `crop_size=None` (the default) to disable cropping and simply
    delegate to `base_loss` unchanged.
    """

    def __init__(self, base_loss: nn.Module, crop_size: Optional[int] = None):
        super().__init__()
        self.base_loss = base_loss
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        if self.crop_size:
            pred = TF.center_crop(pred, self.crop_size)
            target = TF.center_crop(target, self.crop_size)
        return self.base_loss(pred, target)


class LUPINJointLoss(nn.Module):
    """
    Combined data + motion-field-conservation loss for the LUPIN joint
    stages (`model=lupin/final` and `model=lupin/direct`).

    Structurally the same physics-informed combination as
    `MotionFieldConservationLoss` (same finite-difference divergence penalty
    on the estimated motion field, via the same Sobel operators), with two
    differences tailored to the full two-network LUPIN model rather than the
    MF-U-Net-only Stage 1:

    1. An optional `crop_size` (see `CenterCropLoss`) is applied to the
       precipitation term only, since border artifacts compound across the
       *two* warp stages LUPIN performs (the Lagrangian-coordinate input
       transform, on top of the persistence extrapolation already present in
       Stage 1).
    2. `pred` is expected to come from `LUPINForecaster` with
       `return_motion_field=True`, i.e. `(precip, motion_field)`, where
       `motion_field` has shape (B, Tout, 2, H, W) — one field per predicted
       step, matching `LUPINForecaster`'s output (Stage 1's
       `MFUNetForecaster` returns the same shape, so the underlying physics
       term implementation is shared/identical).

    Note
    ----
    The reference LUPIN implementation also includes an optional auxiliary
    term that re-checks per-step Lagrangian self-consistency against the raw
    input sequence (`gamma`/`extra_criterion_loss` in `models/LUPIN/LUPIN.py`
    of the reference repository). That term needs the model's *input*, but
    this benchmark's `engine.run_training` calls losses as
    `criterion(pred, y)` (no `x`), so it is intentionally not reproduced
    here — only the `beta`-weighted data + motion-conservation combination
    is. This mirrors the same benchmark-contract-driven simplification
    already made for `MotionFieldConservationLoss` in Stage 1.
    """

    def __init__(self, base_loss: nn.Module, beta: float = 0.1, crop_size: Optional[int] = None):
        super().__init__()
        self.base_loss = base_loss
        self.beta = beta
        self.crop_size = crop_size
    ...
    def forward(self, pred, target):
        precip, motion_field = pred  # <- desempacota a tupla vinda do LUPINForecaster

        if self.crop_size:
            precip = TF.center_crop(precip, self.crop_size)
            target = TF.center_crop(target, self.crop_size)

        data_term = self.base_loss(precip, target)
        ...
        return (1 - self.beta) * data_term + self.beta * physics_term